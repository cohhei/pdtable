...
class PdtableTest < Minitest::Test
  def setup
    ...
    @default_csv = Pdtable::Table.new @default_path
  end
  ...
  def test_it_returns_a_csv_table
    assert_equal @default_csv.class, Pdtable::Table
  end

  def test_it_returns_error_when_invalid_path
    e = assert_raises Errno::ENOENT do
      t = Pdtable::Table.new @invalid_path
    end
  ...
  def test_specified_dtype
    ...
    c = Pdtable::Table.new @default_path, dtype: dtype
  ...
  def test_skip_rows
    del0  = Pdtable::Table.new @default_path, skiprows: 0
    del12 = Pdtable::Table.new @default_path, skiprows: [1, 2]

    c = Pdtable::Table.new @default_path
    c.delete 0
    assert_equal del0, c
